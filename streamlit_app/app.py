...
/* Transparent fullscreen button on charts */
...
with tab_dashboard:
    ...
    # ── DB connection status (helps diagnose empty charts on cloud) ─────────────
    with st.expander("🔌 Connection diagnostics", expanded=False):
        try:
            has_pg_secret = "postgres" in st.secrets
        except Exception:
            has_pg_secret = False
        st.write(f"**st.secrets has 'postgres' key:** `{has_pg_secret}`")
        try:
            engine = _get_engine_cloud()
            with engine.connect() as _c:
                n = _c.execute(text("SELECT COUNT(*) FROM departures")).scalar()
            st.success(f"DB connected — departures rows: {n:,}")
