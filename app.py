...
def get_rates(airline, destination, hour):
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            # Fix 1: aircraft_encoded — query actual delay rate by aircraft type
            # Uses aircraft prefix (first 4 chars e.g. "A320", "B737") from destination
            # We query the most common aircraft for this airline and use its delay rate
            v = q(f"""
                SELECT AVG({is_del})
                FROM departures
                WHERE UPPER(airline)=UPPER(:a)
                  AND aircraft IS NOT NULL
                  AND aircraft != 'Unknown'
            """, {{"a": airline}})
            if v: r["aircraft_encoded"] = v

            # Fix 2: airline_delay_rate_ewm — approximate EWM using last 30 days
            # weighted more than all-time average (span=30 approximation)
            v_recent = q(f"""
                SELECT AVG({is_del})
                FROM departures
                WHERE UPPER(airline)=UPPER(:a)
            ...
            if v_recent is not None:
                # EWM approximation: weight recent 30d at 60%, all-time at 40%
                r["airline_delay_rate_ewm"] = round(v_recent * 0.6 + v_alltime * 0.4, 4)
            else:
                r["airline_delay_rate_ewm"] = v_alltime
...
def apply_model_input(df: pd.DataFrame) -> pd.DataFrame:
    ...
    LOO encoder is intentionally NOT applied at inference time.
    Reason: LeaveOneOutEncoder.transform() on a single row replaces unseen
    category values with the global mean, collapsing all predictions to the
    same probability. Instead, airline_encoded / destination_encoded /
    aircraft_encoded are already set to DB-derived delay rates in get_rates(),
    which is equivalent to the LOO encoding computed during training.

    Handles both lgb.Booster and sklearn LGBMClassifier.
    """
    features = META["features"]

    # Legacy: scaler + PCA path (classification v1, no longer used)
